...
const UPLOAD_DIR = path.join(process.cwd(), "public", "life-dice", "uploads");
...
function generateFilename(originalName: string | undefined, mime: string) {
  const safeName = (originalName ?? "life-die").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  const suffix = Math.random().toString(36).slice(2, 8);
  const base = safeName ? `${safeName}-${suffix}` : `life-die-${suffix}`;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json({ error: "File too large (4MB max)" }, { status: 400 });
    }
    ...
    const publicPath = `/life-dice/uploads/${filename}`;
    return NextResponse.json({ imagePath: publicPath });
  } catch (error) {
    console.error("POST /api/life-dice/upload failed", error);
